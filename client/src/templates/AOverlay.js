...
export default function AOverlay(props) {
  ...
  function updateCG() {
    ...
    props.socket.emit("updateCG", 42, data);
  }
      ...
      <div className="inputDiv" style={{ flexDirection: "column" }}>
        {state.inputs.map((value, index) => (
          <input
            ...
            value={value}
            onChange={(e) =>
              dispatch({ type: "change", input: index, value: e.target.value })
            }
          />
        ))}
        {state.canExpand && (
          ...
          >
            <button
              ...
              style={{ width: "1.5vw", height: "1.4vw", fontSize: ".8vw" }}
            >
              +
            </button>
            <button
              onClick={() => dispatch({ type: "remove" })}
              disabled={state.numberOfInputs === 0}
              style={{ width: "1.5vw", height: "1.4vw", fontSize: ".8vw" }}
            >
              -
            </button>
